...
export const ControlPanel: React.FC<ControlPanelProps> = ({
  ...
  return (
    <div className="p-8 flex flex-col h-full space-y-12">
      ...
      <div className="flex flex-col gap-10 flex-1 py-10 h-full ">
        {controls.map((ctrl) => {
          ...
          return (
            <div
              key={ctrl.prop}
              className="group flex flex-col gap-6 bg-white p-7 rounded-xl shadow-sm border border-slate-200/60 hover:shadow-md transition-all duration-300"
            >
              <div className="flex justify-between items-center">
                ...
                {!isColor && isStyleObj && (
                  <div className="flex items-center gap-3">
                    <div className="flex bg-slate-100 p-0.5 rounded-lg text-[9px] font-bold tracking-wider border gap-1 border-slate-200/50">
                      {(['px', 'rem', 'em'] as CssUnit[]).map((u) => (
                        <button
                          ...
                          className={`px-3 py-1.5 rounded-md transition-all ${currentUnit === u ? 'bg-white text-blue-600 shadow-sm ring-1 ring-slate-200' : 'text-slate-400 hover:text-slate-600'}`}
                        >
                          {u}
                        </button>
                      ))}
